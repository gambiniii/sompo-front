...
export const assessCombinedRisk = async ({
  route,
  expected_hour,
  weekday,
  cargo_type,
  weather_condition,
}: Props): Promise<CombinedRiskResponse> => {
  const response = await sompoApiWithoutToken.post<CombinedRiskResponse>(
    '/combined-risk/route/assessment',
    {
      route,
      expected_hour,
      weekday,
